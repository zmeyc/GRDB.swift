...
class BelongsToRelationTests: GRDBTestCase {
    ...
    func testInclude() {
        assertNoError {
            ...
            let relation = ForeignRelation(name: "owner", tableName: "owner", foreignKey: ["ownerID": "id"])
    ...
    func testRecursiveInclude() {
        assertNoError {
            ...
            let relation = ForeignRelation(name: "friend", tableName: "persons", foreignKey: ["friendID": "id"])
            let request = rootTable.include(relation)
