...
class HasOneRelationTests: GRDBTestCase {
    ...
    func testJoin() {
        assertNoError {
            ...
            let relation = ForeignRelation(name: "owned", tableName: "owned", foreignKey: ["id": "ownerID"])
    ...
    func testRecursiveJoin() {
        assertNoError {
            ...
            let relation = ForeignRelation(name: "friend", tableName: "persons", foreignKey: ["id": "friendID"])
            let request = rootTable.include(relation)
